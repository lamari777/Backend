...
async def get_categories_by_business(conn: asyncpg.Connection, id_business: int) -> list[dict]:
    rows = await conn.fetch(
        "SELECT id_category, name_category, description_category, id_parent_category, id_business FROM Category WHERE id_business = $1", 
        id_business
    )
...
async def get_category_by_id(conn: asyncpg.Connection, id_category: int, id_business: int) -> Optional[dict]:
    row = await conn.fetchrow(
        "SELECT id_category, name_category, description_category, id_parent_category, id_business FROM Category WHERE id_category = $1 AND id_business = $2",
...
async def create_category(conn: asyncpg.Connection, category: CategoryCreate, id_business: int) -> dict:
    row = await conn.fetchrow(
        "INSERT INTO Category (name_category, description_category, id_parent_category, id_business) VALUES ($1, $2, $3, $4) RETURNING id_category, name_category, description_category, id_parent_category, id_business",
...
async def update_category(conn: asyncpg.Connection, id_category: int, category: CategoryUpdate, id_business: int) -> Optional[dict]:
    row = await conn.fetchrow(
        ...
        RETURNING id_category, name_category, description_category, id_parent_category, id_business
        """,
