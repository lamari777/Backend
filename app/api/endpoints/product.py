...
@router.get("/{batch_number}", response_model=ProductOut, summary="Obtener un producto por ID")
async def obtener_producto(
    ...
    id_business = int(current_payload["sub"])
    product = await product_repo.get_product_by_id(conn, id_business, batch_number)
    if not product:
...
@router.patch("/{batch_number}", response_model=ProductOut, summary="Actualizar un producto")
async def actualizar_producto(
    ...
    id_business = int(current_payload["sub"])
    try:
        updated = await product_repo.update_product(conn, id_business, batch_number, product)
        if not updated:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Producto no encontrado o no pertenece a tu negocio")
        return updated
    except asyncpg.exceptions.UniqueViolationError:
...
@router.delete("/{batch_number}", status_code=status.HTTP_204_NO_CONTENT, summary="Eliminar un producto")
async def eliminar_producto(
    ...
    id_business = int(current_payload["sub"])
    deleted = await product_repo.delete_product(conn, id_business, batch_number)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Producto no encontrado o no pertenece a tu negocio")